function [dq_t, ddq_t, Tau, t_sample]=timeOptimSpeedPlan_LP(q_s, dq_s, ddq_s, h, M_fn, C_fn, G_fn, constraints)
% TIMEOPTIMSPEEDPLAN_LP time optimal speed planning with kinematic and dynamic constraints 
% by solving LP using dual-simplex. The problem should satisfy several assumptions to form the structure: 
%   (1) max torque>= g term;
...
% generate inequality matrices & upper bound
 ...
 %form A, b, Aeq, beq for LP
[A, b, Ub]=get_LPIneq(D_d, D_c, D_g, D_mu, D_dgamma, D_ddgamma, D_alpha, h, Ub);

Aeq = zeros(2, N);
Aeq(1, 1) = 1;
Aeq(2, end) = 1;
beq = [constraints.b_init; constraints.b_end];
...
tic;
b_optim = linprog(f, A, b, Aeq, beq, Lb, Ub, options);
solveTime = toc;
